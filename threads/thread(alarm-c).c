...
void thread_sleep(int64_t ticks) {
	struct thread *curr;					// 현재 스레드를 가리킬 포인터
	enum intr_level old_level;		// 인터럽트 레벨을 저장할 변수

	old_level = intr_disable();		// 인터럽트 비활성화
	curr = thread_current();			// 현재 스레드
	ASSERT (curr != idle_thread);	// 현재 스레드가 idle이 아닐 때
	
	curr->wakeup_ticks = ticks;		// 스레드가 깨울 틱 수를 wakeup_ticks에 저장
	list_insert_ordered(&sleep_list, &curr->elem, cmp_thread_ticks, NULL);	// sleep_list에 추가
	thread_block();								// 현재 스레드 재우기 (blcok)
